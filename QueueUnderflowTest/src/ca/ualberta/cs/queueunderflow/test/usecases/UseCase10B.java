...
//Use CASE 10 (incorporates user stories 10) - Testing sort by MOST RECENT QUESTIONS display
public class UseCase10B extends ActivityInstrumentationTestCase2<MainActivity> {
	...
	public void testSortByMostRecentDisplay() throws Throwable {
		getInstrumentation().invokeMenuActionSync(getActivity(), R.id.mostRecentMenu, 0);	// Perform click on the sortby most recent menu item to sort questions
		...
		runTestOnUiThread(new Runnable() {
			...
			public void run() {
				...
				// Check that the questions are displayed in sorted order of most recent
				// Check that the 1st question from the top is the question3 the most recent
				assertEquals(question3, adapter.getItem(0));
				...
				assertEquals(DateFormat.format("MMM dd, yyyy", question3.getDate()), dateDisplay1.getText().toString());
				...
				// Check that the 3rd question from the top is the question1 with 0 (the 3rd most / least) recent
				assertEquals(question1, adapter.getItem(2));
				...
				assertEquals(DateFormat.format("MMM dd, yyyy", question1.getDate()), dateDisplay3.getText().toString());
			}
		});
